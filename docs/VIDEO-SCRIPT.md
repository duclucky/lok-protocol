...
| 2:10-2:30 | Two wallets use the same Claim / check prize action  | “Every participant receives the same encrypted credit path and the same ACL pattern. Losers decrypt zero; exactly one participant decrypts the prize. A winner-only claim would publish the winner, so Lok’s claim action is a private EIP-712 decrypt, not a public winner-only transaction.”                                                                          |
...
5. The identical “Claim / check prize” action for two participant wallets.
